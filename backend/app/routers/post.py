...
def create_post(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    destination = db.query(models.Destination).get(destination_id)

    if destination is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Destination not found")

    new_post = models.Post(owner_id=current_user.id, dest=destination, **post.dict(exclude={"destination_id"}))
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
    return new_post
...
def update_post(id: int, updated_post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if updated_post.destination_id is not None:
        # Provjerite postoji li destinacija s novim ID-om i dodijelite je postu
        destination = db.query(models.Destination).filter(models.Destination.id == updated_post.destination_id).first()
        if destination is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Destination with id {updated_post.destination_id} does not exist")
        post.dest_id = destination.id
